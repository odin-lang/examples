...
main :: proc() {
	num_renderers := SDL.GetNumRenderDrivers()
	fmt.println("Renderers:")
	for i in i32(0)..<num_renderers {
		fmt.printfln("\t%s", SDL.GetRenderDriver(i))
	}
	...
	if !SDL.CreateWindowAndRenderer("microui-odin", 960, 540, SDL.WINDOW_RESIZABLE, &state.window, &state.renderer) {
	...
	fmt.printfln("Selected renderer: %s", SDL.GetRendererName(state.renderer))

	if !SDL.SetRenderVSync(state.renderer, 1) {
		fmt.eprintfln("Couldn't set render vsync: %s", SDL.GetError())
		return
	}

	state.atlas_texture = SDL.CreateTexture(state.renderer, .RGBA32, .TARGET, mu.DEFAULT_ATLAS_WIDTH, mu.DEFAULT_ATLAS_HEIGHT)
	assert(state.atlas_texture != nil)
	if !SDL.SetTextureBlendMode(state.atlas_texture, {.BLEND}) {
		fmt.eprintln("SDL.SetTextureBlendMode: %s", SDL.GetError())
		return
	}
	...
	//               widget or class of widget has focus, but it thankfully ignores spurious text edit
	//               events whenever a `mu.textbox` doesn't have focus, so we can just enable it globally.
	//
	//               A well-designed application may want to consider selectively enabling these events
	//               only when a text box has focus.
	if !SDL.StartTextInput(state.window) {
		fmt.eprintfln("SDL.StartTextInput: %s", SDL.GetError())
		return
	}
	...
	if !SDL.AddEventWatch(proc "c" (data: rawptr, event: ^SDL.Event) -> bool {
		...
		fmt.eprintfln("SDL.AddEventWatch: %s", SDL.GetError())
	}
	...
	main_loop: for {
		...
		for e: SDL.Event; SDL.PollEvent(&e); /**/ {
			...
			case .MOUSE_BUTTON_DOWN, .MOUSE_BUTTON_UP:
				fn := mu.input_mouse_down if e.button.down else mu.input_mouse_up
				switch e.button.button {
				case 1: fn(ctx, i32(e.button.x), i32(e.button.y), .LEFT)
				case 3: fn(ctx, i32(e.button.x), i32(e.button.y), .MIDDLE)
				case 2: fn(ctx, i32(e.button.x), i32(e.button.y), .RIGHT)
				}
			...
			case .KEY_DOWN, .KEY_UP:
				...
				fn := mu.input_key_down if e.key.down else mu.input_key_up
...
render :: proc "contextless" (state: ^State) {
	render_texture :: proc "contextless" (renderer: ^SDL.Renderer, dst: ^SDL.FRect, src: mu.Rect, color: mu.Color) {
		dst.w = f32(src.w)
		dst.h = f32(src.h)

		SDL.SetTextureAlphaMod(state.atlas_texture, color.a)
		SDL.SetTextureColorMod(state.atlas_texture, color.r, color.g, color.b)
		SDL.RenderTexture(renderer, state.atlas_texture, &SDL.FRect{f32(src.x), f32(src.y), f32(src.w), f32(src.h)}, dst)
	}

	viewport_rect := &SDL.Rect{}
	SDL.GetRenderOutputSize(state.renderer, &viewport_rect.w, &viewport_rect.h)
	SDL.SetRenderViewport(state.renderer, viewport_rect)
	SDL.SetRenderClipRect(state.renderer, viewport_rect)
	SDL.SetRenderDrawColor(state.renderer, state.bg.r, state.bg.g, state.bg.b, state.bg.a)
	...
	for variant in mu.next_command_iterator(&state.mu_ctx, &command_backing) {
		...
		case ^mu.Command_Rect:
			...
			SDL.RenderFillRect(state.renderer, &SDL.FRect{f32(cmd.rect.x), f32(cmd.rect.y), f32(cmd.rect.w), f32(cmd.rect.h)})
		case ^mu.Command_Icon:
			src := mu.default_atlas[cmd.id]
			x := cmd.rect.x + (cmd.rect.w - src.w)/2
			y := cmd.rect.y + (cmd.rect.h - src.h)/2
			render_texture(state.renderer, &SDL.FRect{f32(x), f32(y), 0, 0}, src, cmd.color)
		case ^mu.Command_Clip:
			SDL.SetRenderClipRect(state.renderer, &SDL.Rect{cmd.rect.x, cmd.rect.y, cmd.rect.w, cmd.rect.h})
