...
os_init :: proc() {
	...
	state.os.window = sdl2.CreateWindow(
...
os_run :: proc() {
	...
	sdl2.DestroyWindow(state.os.window)
	sdl2.Quit()
...
os_get_render_bounds :: proc() -> (width, height: u32) {
	iw, ih: c.int
	sdl2.GetWindowSize(state.os.window, &iw, &ih)
	return u32(iw), u32(ih)
}

os_get_surface :: proc(instance: wgpu.Instance) -> wgpu.Surface {
	return sdl2glue.GetSurface(instance, state.os.window)
}
